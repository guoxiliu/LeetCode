/*
 * @lc app=leetcode id=2122 lang=cpp
 *
...
public:
    vector<int> recoverArray(vector<int>& nums) {
        ...
        // try each possible positive integer k
        for (int i = 1; i < n; i++) {
            int diff = nums[i] - nums[0];
            // the difference needs to be even and greater than 0
            if ((diff & 1) || diff == 0) continue;
            // check if it is a possible k 
            int k = diff / 2;
