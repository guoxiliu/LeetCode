...
public:
  vector<int> prisonAfterNDays(vector<int>& cells, int N) {
     ...
     * The length of the array is fixed. The first and the last bit
     * will always be zero after one day. So only the middle 6 bits 
     * needs to be computed, and only 2^6 = 64 states existed.
     * Since the cycle is 14, you can reduce the times with (N-1) % 14 + 1.
     */
    vector<int> tmp(8);
